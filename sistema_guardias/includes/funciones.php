<?php
// Cargar todas las funciones
require_once __DIR__.'/funciones/funciones_autenticacion.php';
require_once __DIR__.'/funciones/funciones_guardias.php';
require_once __DIR__.'/funciones/funciones_usuarios.php';
require_once __DIR__.'/funciones/funciones_personal.php';
require_once __DIR__.'/funciones/funciones_ui.php';
require_once __DIR__.'/funciones/funciones_novedades.php';
require_once __DIR__.'/funciones/funciones_vehiculos.php';